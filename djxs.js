...
function djxsvideo(timeout = 0) {
    return new Promise((resolve) => {
        ...
        "Referer": djxsReferer,
        "User-Agent": "Mozilla/5.0 (Linux; Android 7.1.2; VOG-L29 Build/NZH54D; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/81.0.4044.117 Mobile Safari/537.36",
        "X-Requested-With": "XMLHttpRequest"}
        dreferfer=djxsReferer.match(/index?(\S.+)/)[1]
        let url = {
            url: `https://dj.palmestore.com/zycl/gold/receive?${dreferfer}&type=videoTask&videoId=10057`,
            headers: hd,
        ...
        $.get(url, async (err, resp, data) => {
            try {
                ...
                if (result.code == 0) {
                    ...
                    console.log(`【开宝箱失败，领取时间未到】：返回信息${result.code}:${result.msg}\n`)
                } else {
...
function djxsxrbox(timeout = 0) {
    return new Promise((resolve) => {
        ...
        dreferfer=djxsReferer.match(/usr=.+/)[0];
        let url = {
            url: `https://dj.palmestore.com/zycl/regression/receive?${dreferfer}&incrId=3&days=2`,//修改days=的值，3-7领取所有新人红包
            headers: hd,
        ...
        $.get(url, async (err, resp, data) => {
            try {
                ...
                if (result.code == 0) {
                    console.log(`【开新人宝箱】：${result.msg}\n`)
                    console.log(`【获得金币】：${result.body}\n`)
                    await $.wait(12000)
                    await djxsboxdb()
                }else if(result.code == 40002){
                    console.log(`【开新人宝箱失败】：返回信息${result.code}:${result.msg}\n`)
                } else {
                    console.log(`【开新人宝箱失败】：${result.msg}\n`)
